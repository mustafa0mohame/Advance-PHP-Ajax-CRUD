...
if ($_GET["action"] === "insertData") {
  if (!empty($_POST["first_name"]) && !empty($_POST["last_name"]) && !empty($_POST["email"]) && !empty($_POST["country"]) && !empty($_POST["gender"]) && $_FILES["image"]["size"] != 0) {
    ...
    $sql = "INSERT INTO `addusers`(`id`, `first_name`, `last_name`, `email`, `image`, `country`, `gender`) VALUES (NULL,'$first_name','$last_name','$email','$new_name','$country','$gender')";
...
if ($_GET["action"] === "updateData") {
  if (!empty($_POST["first_name"]) && !empty($_POST["last_name"]) && !empty($_POST["email"]) && !empty($_POST["country"]) && !empty($_POST["gender"])) {
    ...
    $sql = "UPDATE `addusers` SET `first_name`='$first_name',`last_name`='$last_name',`email`='$email',`image`='$new_name',`country`='$country',`gender`='$gender' WHERE `id`=$id";
    if (mysqli_query($conn, $sql)) {
      echo json_encode([
        "statusCode" => 200,
...
if ($_GET["action"] === "deleteData") {
  ...
  $sql = "DELETE FROM addusers WHERE `id`=$id";
